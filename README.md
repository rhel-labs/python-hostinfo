...
* **JSON Configuration**: Add or remove monitored packages by editing a simple `app/config.json` file.
...
1.  **Clone & Setup**:
    ```bash
    git clone https://github.com/rhel-labs/python-hostinfo.git
    cd python-hostinfo
    python3 -m venv venv
...
1.  **Build the Image**:
    ...
    podman build -t host-details-app .
    ```
2.  **Run the Container**:
    ```bash
...
This is the easiest customization. Simply open `app/config.json` and edit the lists.
...
**File to Edit**: `app/templates/index.html`

Let's say you want to change the main header icon.

1.  Find this line in `app/templates/index.html`:
    ```html
...
You can apply this same process to any icon in the `app/templates/index.html` file or the `app/static/main.js` file where icons are dynamically generated.
...
**File to Edit**: `app/static/main.js`
